...
const addReview=async(user:reviewUser,payload:reviewType)=>{
  const findOrder=await prisma.order.findFirst({where:{customerId:user?.id,status:ORDERSTATUS.DELIVERED}});
  if(!findOrder){
  ...
  const addReviewData=await prisma.review.create({data:payload});
  if(addReviewData){
...
const editReview=async(id:string,user:reviewUser,payload:reviewUpdateType)=>{
    const findOrder=await prisma.order.findFirst({where:{customerId:user?.id,status:ORDERSTATUS.DELIVERED}});
    if(!findOrder){
      return {success:false,message:"Order not found to review",data:null}
    }
    ...
    const addReviewData=await prisma.review.update({where:{id:id},data:payload});
    if(addReviewData){
        const result = await prisma.review.aggregate({
            where: { mealId:payload?.mealId },
            _avg: { rating: true },
...
const getAllReview=async(mealId:string)=>{
    const findAllReview=await prisma.review.findMany({where:{mealId:mealId}});
...
const deleteReview=async(id:string,user:reviewUser)=>{
  const findReview=await prisma.review.findFirst({where:{id:id,userId:user?.id}});
  if(findReview){
...
export const reviewService={
    addReview,editReview,getAllReview,deleteReview
}